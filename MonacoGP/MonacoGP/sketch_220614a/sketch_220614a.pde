...
float start_x_1 = 130; //x für Auto 1 -Spieler
float start_x_2 = 170; //x für Auto 2
float start_x_3 = 300; //x für Auto 3
...
int Auto_width = 60;  //Auto breite (alle Autos)
int Auto_height = 113; //Auto höhe  (alle Autos)

int ScreenSize_width = 450; //Display breite
int ScreenSize_height = 700; //Display höhe 
...
void setup() {
     size(450,700);  //Display Größe einstellen
     //Bilder
...
void game_running() {
  //Aufbau der Spielseite
            ...
            y_0 = y_0 -1.3;
            if(y_0+90<1) { //Wenn das Auto den obersten Teil erreicht hat fängt es wieder von vorne an
            y_0= 650;
            }
            ...
            if(mousePressed) {
               y_1 = y_1 -3 ; //Wenn maus gedrückt wird, wird die Geschwindigeit hoch gesetzt
             } else {
              y_1 = y_1 -0.3 ;  //Wenn maus nicht gedrückt wird, wird die Geschwindigkeit auf 0.3 gesetzt
            }
           
            if(y_1+90<1) { //Erkennung oberer Teil
              y_1 = 650;
            }            
           
            image(LilaAuto,x_2,y_2, Auto_width,Auto_height);  //Bot auto 2
            y_2 = y_2 -2.5;
            if(y_2+90<1) { //Erkennung oberer Teil
                y_2 = 650;
            }
           
            image(LilaAuto,x_3,y_3, Auto_width,Auto_height);  //Bot auto 3
            y_3--;
            if(y_3+90<1) { //Erkennung oberer Teil
               y_3 = 650;
            }
